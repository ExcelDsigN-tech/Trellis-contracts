...
fn example_simulation_multi_user_payments() {
    ...
    // Create token
    let (token_addr, mut token_client, asset_client) =
        env.register_stellar_asset_contract_with_client(admin.clone());
...
fn example_fuzz_access_control() {
    ...
    assert!(
        results.caught_violations == results.unauthorized_attempts,
...
impl MockTreasury {
    ...
    pub fn deposit(env: Env, from: Address, amount: i128) {
        ...
        env.storage()
            .persistent()
            .set(&b"balance", &treasury_balance);
    }
