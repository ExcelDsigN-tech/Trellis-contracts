...
use storage::{get_aid, get_aid_counter, has_aid, set_aid, set_aid_counter, set_token};
...
impl AidContract {
    ...
    /// Initialise the contract, storing the admin address and the token used
    /// for all escrow transfers.
    ///
    ...
    ///
    /// Transfers `amount` of the configured token from `donor` to this
    ...
    /// - [`AidError::Expired`]        — expiry has not yet passed.
    ...
    pub fn set_paused(env: Env, caller: Address, paused: bool) {
        if let Err(err) = shared::auth::require_admin(&env, &caller) {
